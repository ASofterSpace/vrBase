...
public abstract class PrettyDome2Ctrl {
	...
	public PrettyDome2Ctrl(MainCtrl mainCtrl, GameObject thisRoom) {
	...
	protected List<Vector3> prettyDome2Coords(float DomeRadius, int spiralturns) {
		...
		for (int j=0; j<=Levels; j++ ) {
			...
			double phi = Math.PI * (j*(360/resolution) + offset)/ 180.0f;
	...
	protected virtual void createBeams() {
		...
		var LevelListi = new List<Vector3>( prettyDome2Coords(6.0f, 10)  );
  ...
  */
	...
	*/
	protected GameObject connectBeam(Vector3 startpoint, Vector3 endpoint) {
		...
		float CornerSphereThickness = 1*BeamThickness;
